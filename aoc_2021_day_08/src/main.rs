...
//fn parser(path: &Path) -> io::Result<Vec<(Vec<String>, Vec<String>)>> {
//    let file = File::open(path)?;
//    let buf = BufReader::new(file);
//
//    let mut result = vec![];
...
//        let parts = line.split_terminator(" | ").collect::<Vec<&str>>();
//
//        let patterns = parts[0].split_whitespace().map(|s| s.to_string()).collect::<Vec<String>>();
//
//        let output = parts[1].split_whitespace().map(|s| s.to_string()).collect::<Vec<String>>();
//
//        result.push((patterns, output));
//    }
//
//    Ok(result)
//}
//fn solve(part_1: bool, lines: &Vec<(Vec<String>, Vec<String>)>) -> u32 {
//    let mut result = 0;
//
//    for line in lines {
//        let (patterns, output) = line;
...
//
//        for (i, code) in output.iter().rev().enumerate() {
//            let code = standardise_code(&code);
//
//            if part_1 {
//                if decoder.contains_key(&code) {
//                    result += 1;
//                }
//            } else {
//                let val = decoder.get(&code).unwrap_or(&0);
...
//}
//
//fn standardise_code(code: &str) -> String {
//    let mut chars = code.chars().collect::<Vec<char>>();
//    chars.sort();
//    chars.iter().collect::<String>()
//}
//
//fn analyse_patterns(patterns: &Vec<String>) -> HashMap<u8, u32> {
//    let mut result = HashMap::new();
//    let mut length_map: HashMap<usize, Vec<String>> = HashMap::new();
//
//    for p in patterns {
//        match p.len() {
//            2 => { result.insert(standardise_code(&p), 1); },
//            3 => { result.insert(standardise_code(&p), 7); },
//            4 => { result.insert(standardise_code(&p), 4); },
//            7 => { result.insert(standardise_code(&p), 8); },
//            _ => {
//                let code = standardise_code(&p);
//                if let Some(pattern_vec) = length_map.get_mut(&p.len()) {
//                    pattern_vec.push(code);
//                } else {
//                    length_map.insert(p.len(), vec![code]);
//                }
//            },
//        }
//    }
